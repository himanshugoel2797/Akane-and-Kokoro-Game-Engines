...
const vec4 cons34 = vec4(0.033591, 0.033591, 0.033591, 0.000000);
const float cons39 = float(6.000000);
const float cons42 = float(0.379699);
const vec3 cons48 = vec3(0.200509, 0.200509, 0.200509);
const float cons61 = float(1.000000);
...
void main(void)
{
	...
	//TODO lamp_visibility_sun_hemi figure out what first parameter does

	shade_norm(facingnormal, tmp2);
	shade_view(varposition, tmp4);
	set_rgb_zero(tmp5);
	set_rgb_zero(tmp6);
	lamp_visibility_sun_hemi(vec3(0,0.5f,0.5f), tmp8, tmp9, tmp10);
	shade_inp(tmp2, tmp8, tmp13);
	shade_visifac(tmp13, tmp10, cons16, tmp17);
	shade_mul_value_v3(1, vec3(1), tmp20);
	shade_mul_value(tmp17, vec4(tmp20, 1.0), tmp23);
	mtex_value_invert(cons24, tmp25);
	mix_mult(tmp25, tmp23, cons28, tmp29);
	mtex_value_invert(tmp25, tmp31);
	shade_madd(vec4(tmp5, 1.0), tmp29, cons34, tmp35);
	shade_cooktorr_spec(tmp2, tmp8, tmp4, cons39, tmp40);
	shade_spec_t(tmp31, cons42, tmp10, tmp40, tmp45);
	shade_add_spec(tmp45, tmp20, cons48, tmp49);
	shade_add_clamped(vec4(tmp6, 1.0), vec4(tmp49, 1.0), tmp52);
	shade_clamp_positive(tmp52, tmp54);
	shade_clamp_positive(tmp35, tmp56);
	shade_add(tmp56, tmp54, tmp59);
	mtex_alpha_to_col(tmp59, cons61, tmp62);
	shade_alpha_opaque(tmp62, tmp64);
	linearrgb_to_srgb(tmp64, tmp66);
